...
impl crate::Storage for PostgresStorage {
    ...
    async fn query_logs_paginated(&self, filter: &LogFilter, page: i64, page_size: i64) -> Result<PaginatedResponse<AuditLogSummary>, Box<dyn std::error::Error + Send + Sync>> {
        // Bind since/until as typed DateTime<Utc> (not to_rfc3339() strings) so
        // postgres sees timestamptz, not text — otherwise `created_at >= $N`
        // fails with "operator does not exist: timestamp with time zone >= text".
        let mut conditions = Vec::new();
        let mut param_idx = 1;
